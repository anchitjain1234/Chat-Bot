...
module.exports = (robot) ->
  robot.respond /what\s+(is|are)\s+([^\?]+)\??$/i, (res) ->
    the_query = res.match[2].trim().replace(/^((a|an|the)\s*)?/, '')
    abstract_url = "http://api.duckduckgo.com/?format=json&q=#{encodeURIComponent(the_query)}"
